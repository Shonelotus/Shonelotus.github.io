class Campo
{
    ...
    creaCampo()
    {
        let tabella = "<table id='tabellaCampo'>";
			for (var i = 0; i < this.numRighe; i++) 
            {
				...
				for (var j = 0; j < this.numColonne; j++) 
                {
					tabella += "<td><button class ='coperta' onclick='campo.scopriCasella(" + i + ', ' + j + ")'></button></td>";
				}
				tabella += "</tr>";
                this.contatore++;
			}
    ...
    scopriCasella(riga, colonna)
    {
        if(!this.isGiocoFinito) //entro se non ho finito il gioco
        {
            ...
            for(let i = 0; i < this.numRighe; i++)
            {
                for(let j = 0; j < this.numRighe; j++)
                {
                    if(this.campo[riga][colonna] != 0)
                    {
                        document.getElementsByTagName("tr")[riga].getElementsByClassName("coperta")[colonna].className = "scoperta"
                        document.getElementsByTagName("tr")[riga].getElementsByTagName("button")[colonna].innerHTML = this.campo[riga][colonna];
                    }
                    
                    this.scopriAdiacenti(riga, colonna);
    ...
    {
        document.getElementsByTagName("tr")[riga].getElementsByTagName("button")[colonna].innerHTML = this.campo[riga][colonna];
        if(riga-1 >= 0 && riga+1 < this.numRighe && colonna-1 >= 0 && colonna+1 < this.numRighe)
        {
            document.getElementsByTagName("tr")[riga].getElementsByTagName("button")[colonna].innerHTML = this.campo[riga][colonna];
            if(this.campo[riga][colonna-1] != "bomba")
            {
                this.scopriAdiacenti(riga, colonna-1);
            }
            if(this.campo[riga+1][colonna-1] != "bomba")
            {
                this.scopriAdiacenti(riga+1, colonna-1);
            }
            if(this.campo[riga+1][colonna+1] != "bomba")
            {
                this.scopriAdiacenti(riga, colonna-1);
            ...
            if(this.campo[riga-1][colonna] != "bomba")
            {
                this.scopriAdiacenti(riga-1, colonna);
            }
            if(this.campo[riga-1][colonna-1] != "bomba")
            {
                this.scopriAdiacenti(riga-1, colonna-1);
